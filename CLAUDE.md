...
**Ассистент** — единая точка входа `services/assistant/orchestrator.run(settings, db, user, message)`
...
(иначе уточняющий вопрос) → роутинг по интенту → сборка карточек и действий.
...
светлая/тёмная через `:root[data-theme]`) + один `app/static/js/app.js` (модули включаются по
наличию якорного DOM-элемента; чат рендерит карточки по `kind` и кнопки по `type`).
...
`{{ base }}/...`, `app.js` префиксует запросы через `window.APP_BASE`, редиректы в роутерах —
