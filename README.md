...
- **Календарь**: день/неделя/месяц, цвет по приоритету, бейджи online/offline/hybrid и
  «приглашён» (участник видит чужие встречи, куда его позвали), линия текущего времени,
  затенение нерабочих часов, клик по сетке создаёт встречу, переход к дате, автокомплит
  участников, «найти свободный слот»; обновление без перезагрузки страницы.
...
- **Ассистент-оркестратор**: нормализация запроса → интент → сервис → карточки и действия
  с подтверждением (включая отмену встречи со сменой статуса — не удаление). Просроченные
  черновики истекают, конфликты перепроверяются при подтверждении. Работает без ключей
  (локальный нормализатор), готов к Dify.
...
- **Уведомления**: mock-канал (пишутся в БД, видны в топбаре и на странице), точечное
  чтение и deep-link на день встречи; фоновая отправка напоминаний
  (`notifications.reminders_enabled`). Задел под мессенджер/email.
...
- **UI**: liquid glass, адаптивный sidebar/topbar, тёмная/светлая тема (по умолчанию —
  системная), SVG-иконки, дизайн-токены, доступность (focus-visible, aria-разметка,
  reduced-motion, контраст AA), скелетоны, тосты, единый confirm-диалог, печать.
...
app/
  ...
  static/js/           # core.js (общий слой) + calendar/chat/documents/travel/admin.js
config/
...
## ⏱ Часовые пояса (соглашение MVP)

Все `datetime` в БД — **наивные, в локальном времени `app.timezone`** (по умолчанию
Europe/Moscow). «Сейчас» на сервере считается через `app/core/clock.py:local_now()`
(корректно даже при другом системном поясе сервера). Браузер разбирает наивные ISO-строки
как локальные и показывает те же настенные часы. Полный переход на aware-UTC —
осознанно за рамками MVP; при выходе на пользователей в разных поясах потребуется
миграция на UTC + конвертация на клиенте.

## ⚠️ Известное ограничение окружения: passlib и Python 3.13
